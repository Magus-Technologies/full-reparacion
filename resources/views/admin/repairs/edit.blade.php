...
<div class="container-fluid px-4">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Editar Reparación</h2>
        ...
        <form id="repair-form" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="client_id" id="client_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ $repair->client_id == $client->id ? 'selected' : '' }}>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="text" id="client_dni" value="{{ $repair->client->documentid }}" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" readonly>
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="number" name="discount" id="discount" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="{{ $repair->discount }}" min="0" step="0.01">
                </div>

                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700">Adelanto</label>
                    <input type="number" name="advance" id="advance" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="{{ $repair->advance }}" min="0" step="0.01">
                </div>

                <!-- Observaciones y Notas -->
                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700">Observaciones</label>
                    <textarea name="remarks" rows="3" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ $repair->remarks }}</textarea>
                </div>

                <div class="col-span-2 md:col-span-1">
                    <label class="block text-sm font-medium text-gray-700">Notas</label>
                    <textarea name="notes" rows="3" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ $repair->notes }}</textarea>
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="payment_status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                        <option value="0" {{ $repair->payment_status == 0 ? 'selected' : '' }}>No pagado</option>
                        <option value="1" {{ $repair->payment_status == 1 ? 'selected' : '' }}>Pagado</option>
                    </select>
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
                        <option value="0" {{ $repair->status == 0 ? 'selected' : '' }}>Pendiente</option>
                        <option value="1" {{ $repair->status == 1 ? 'selected' : '' }}>Entregado</option>
                        <option value="2" {{ $repair->status == 2 ? 'selected' : '' }}>En Progreso</option>
                        <option value="3" {{ $repair->status == 3 ? 'selected' : '' }}>Devolución</option>
                        <option value="4" {{ $repair->status == 4 ? 'selected' : '' }}>Realizado</option>
                        <option value="5" {{ $repair->status == 5 ? 'selected' : '' }}>Cancelado</option>
                    </select>
                </div>
            </div>
...
@push('scripts')
<script>
let services = @json($repair->services->map(function($service) {
    return [
        'id' => $service->id,
        'name' => $service->service,
        'cost' => $service->pivot->fee
    ];
}));
let total = {{ $repair->total_amount }};
...
$(document).ready(function() {
    ...
    $('#repair-form').submit(function(e) {
        ...
        $.ajax({
            url: `{{ route('admin.repairs.update', $repair->id) }}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
